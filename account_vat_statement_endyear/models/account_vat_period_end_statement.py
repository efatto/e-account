...
class VatPeriodEndStatementReport(report_sxw.rml_parse):
    ...
    def _compute_tax_amount(self, tax, tax_code, base_code, context=None):
        ...
        if tax.parent_id:
            ...
            for child in tax.parent_id.child_ids:
                if not child.tax_code_id.is_base:
                    # deductibile
                    if child.account_collected_id:
                        if context.get('date_start', False) and \
                                context.get('date_end', False):
                            vat_deductible = child.tax_code_id.sum_date
                        else:
                    ...
                    # undeductibile
                        ...
                        if context.get('date_start', False) and \
                                context.get('date_end', False):
                            vat_undeductible = child.tax_code_id.sum_date
                        else:
            ...
            if context.get('date_start', False) and \
                    context.get('date_end', False):
                vat_deductible = not tax_code.is_base and tax_code.sum_date
            else:
        ...
        res[vat_name] = {
            ...
            'base': context.get('date_start', False) and context.get(
                'date_end', False) and base_code.sum_date or
            base_code.sum_period,
        }
                ...
                if test_base_code.id != base_code.id:
                    raise orm.except_orm(
                        ...
                        _('Not every tax linked to tax code %s is linked to '
                          'the same base code')
                        % tax_code.name)
            if context.get('date_start', False) and context.get(
                    'date_end', False):
                if tax_code.sum_date or base_code.sum_date:
    ...
    def _get_tax_codes_amounts(self, periods, date_start=None, date_end=None,
                               tax_code_ids=None, context=None):
        if context is None:
            context = {}
            ...
            for period_id in periods:
                ...
                for tax, value in res.iteritems():
                    if tax in result:
                        result[tax]['base'] += value['base']
                        result[tax]['vat'] += value['vat']
                        result[tax]['vat_deductible'] += value[
                            'vat_deductible']
                        result[tax]['vat_undeductible'] += value[
                            'vat_undeductible']
                        result[tax]['code'] += value['code']
    ...
    def _get_fy(self, statement):
        ...
        if statement:
            ...
            fiscalyear = self.pool.get('account.fiscalyear').browse(
                self.cr, self.uid, fiscalyears[0])
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        return super(VatPeriodEndStatementReport, self).set_context(
