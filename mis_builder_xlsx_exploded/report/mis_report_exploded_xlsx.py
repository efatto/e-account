...
class MisBuilderXlsxExploded(models.AbstractModel):
    _name = "report.mis_builder_xlsx_exploded.report_exploded_xlsx"
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        for row in matrix.iter_rows():
            ...
            for cell in row.iter_cells():
                ...
                # add a line for every origin object
                expression = row.kpi.expression
                query_name = row.kpi.query_id.name
                if (
                    row.kpi.auto_expand_accounts and
                    cell.drilldown_arg and cell.drilldown_arg.get("domain") and
                    cell.drilldown_arg.get("model")
                ):
                    cell_domain = cell.drilldown_arg.get("domain")
                    cell_model = cell.drilldown_arg.get("model")
                    cell_lines = self.env[cell_model].search(cell_domain)
                    for cell_line in cell_lines:
                        cell_value = safe_eval(expression, {query_name: cell_line})
                        if not cell_value:
                            continue
                        row_pos += 1
                        cell_name = safe_eval(
                        ...
                        sheet.write(
                            row_pos, col_pos - 1, cell_value, cell_value_format)
            row_pos += 1
